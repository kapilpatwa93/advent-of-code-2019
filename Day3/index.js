...
function traverseThroughPath(path) {
    ...
    path.forEach((p) => {
        ...
        for (let i = 0; i < distance; i++) {
            ...
            if (pathMap.get(currentLocation[0] + "." + currentLocation[1]) == 1) {
                if (shortestPath > calculateDistanceFromCenter(currentLocation)) {
                    shortestPath = calculateDistanceFromCenter(currentLocation)
                }
            }
        }
    })
...
function goThroughPath(path) {
    ...
    path.forEach((p) => {
        ...
        for (let i = 0; i < distance; i++) {
            currentLocation[0] += offset[0];
            currentLocation[1] += offset[1];
            pathMap.set(currentLocation[0] + "." + currentLocation[1], 1)
        }
    })
}
