...
function traverseThroughPath(path) {
    ...
    path.forEach((p) => {
        ...
        for (let i = 0; i < distance; i++) {
            ...
            steps++;
            const intersetion = pathMap.get(currentLocation[0] + "." + currentLocation[1])
            if (intersetion) {
                if (lowestSteps > (steps + intersetion.steps) ) {
                    lowestSteps = steps + intersetion.steps
                }
            }
        }
    })
...
function calculateDistanceFromCenter(currLocation) {
    return Math.abs(currLocation[0]) + Math.abs(currLocation[1])
}
function goThroughPath(path) {
    const currentLocation = [0, 0];
    let steps = 0;
    path.forEach((p) => {
        const op = getDirectionAndDistance(p);
        ...
        for (let i = 0; i < distance; i++) {
            ...
            currentLocation[1] += offset[1];
            steps++;
            pathMap.set(currentLocation[0] + "." + currentLocation[1], {
                steps:steps
            })
        }
    })
...
function getDirectionAndDistance(operation) {
    return {
        "direction": operation.substring(0, 1),
        "distance": parseInt(operation.substring(1))
    }
}
